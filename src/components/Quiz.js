...
export default function Quiz(props) {
	...
	const fetchQuizData = async ({questionNumber, category, difficulty, type}) => {
		...
		const questions = getQuestions(quiz.results);
		setQuestionsData(quiz.results);
...
// React hooks
	...
	function getQuestions(questionsData) {
		return (
	...
	const questionElements = questions.map(question => {
		const answerElements = question.answers.map(answer => {
			return (
				<div
				key={answer.id} className='question__response'
				//style={{backgroundColor: `${ answer.isChosen ? '#99a4e3' : 'white'}`}}
				style={{backgroundColor: `${ (!answer.state && 'white')
				...
				>
					<span className='question__response-text'>{decodeStr(answer.text)}</span>
				</div>
			)
		})
	...
	function chooseAnswer(questionId, answerId) {
		...
		setQuestions(prevQuestions => (
			prevQuestions.map(question => {
				const answers = question.answers.map(answer => {
					return question.id === questionId ? {...answer, state: null} : answer
				})
		...
		setQuestions(prevQuestions => (
			prevQuestions.map(question => {
				const answers = question.answers.map(answer => {
					return answer.id === answerId ? {...answer, state: 'chosen'} : answer
				})
	...
	function startNewQuiz() {
		setQuestions(getQuestions(questionsData));
	...
	return(
		...
		{ !loading &&
			<form className='quiz'>
				...
				<button onClick={isFinishedQuiz ? startNewQuiz : checkAnswers} type='button' className='quiz__btn'>{`${isFinishedQuiz ? 'Try again' : 'Show answers'}`}</button>
			</form>
		}
		</>
	)
